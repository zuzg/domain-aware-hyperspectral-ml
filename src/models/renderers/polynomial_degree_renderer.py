...
class PolynomialDegreeRenderer(BaseRenderer):
    ...
    def __call__(self, batch: Tensor) -> Tensor:
        ...
        self.bands = torch.arange(1/100, 151/100, step=0.01, device=self.device).float().repeat(k, h, w, 1)
        rendered = torch.zeros((batch_size, 150, h, w))

        for idx in range(batch_size):
            polynoms = self.generate_polynomial(batch[idx, :, 0, ...], k)
            # print(polynoms.shape)
    ...
    def generate_polynomial(self, coefficient: Tensor, degree: int) -> Tensor:
        exponent = torch.arange(degree, device=self.device).repeat(10, 10, 150, 1).permute(3, 0, 1, 2)
        # print(exponent[:, 0, 0])
        # print("exp", exponent.shape)
