...
class PolynomialDegreeRenderer(BaseRenderer):
    ...
    def __call__(self, batch: Tensor) -> Tensor:
        ...
        self.bands = (
            torch.arange(
                1 / 100, (self.channels + 1) / 100, step=0.01, device=self.device
            )
            .float()
            .repeat(k, self.h, self.w, 1)
        )
        rendered_list = []
    ...
    def generate_polynomial(self, coefficient: Tensor, degree: int) -> Tensor:
        exponent = (
            torch.arange(degree, device=self.device)
            .repeat(self.h, self.w, self.channels, 1)
            .permute(3, 0, 1, 2)
        )
